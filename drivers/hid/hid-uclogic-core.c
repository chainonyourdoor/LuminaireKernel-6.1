...
static void uclogic_remove(struct hid_device *hdev)
{
	...
	/*
	 * uclogic_raw_event_pen() re-arms inrange_timer on every pen report and
	 * keeps doing so until hid_hw_stop() stops the transport, so a report
	 * landing after this delete can arm it once more.  Upstream uses
	 * timer_shutdown_sync() to make such a re-arm a no-op; this tree has no
	 * shutdown variant, so the weaker delete is used and the window stays
	 * open.  See the fixup commit message for the full analysis.
	 */
